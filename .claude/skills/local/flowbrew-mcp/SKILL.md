# Flowbrew MCP Access (raw HTTP, no platform MCP registration)

How Smith talks to the Flowbrew (`taskflow-platform`) MCP server when acting
as its blind-user test agent. This is **local/team-specific** — Flowbrew is
not a generic StepForge concern, so this does not belong in
...
independent POSTs with just the bearer token. So Smith drives it with
`curl` from Bash instead of a registered tool.
